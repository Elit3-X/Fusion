...
const entry = (id: string, focusNonce = 1) => ({ projectId: "project-a", session: { id, agentId: "agent-1", title: id, status: "active" as const, createdAt: "2026-08-21T00:00:00.000Z", updatedAt: "2026-08-21T00:00:00.000Z" }, focusNonce });
...
describe("PoppedOutChatWindows", () => {
  it("renders independent selected chats and closes only the requested entry", () => {
    ...
    render(<PoppedOutChatWindows entries={[entry("a", 2), entry("b", 7), { ...entry("other"), projectId: "project-b" }]} projectId="project-a" addToast={vi.fn()} onClose={onClose} onOpenSessionInNewWindow={onOpenSessionInNewWindow} />);
