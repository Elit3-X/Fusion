...
- Approval resolution is escalate-only: workflow/project `requirePlanApproval=true`, project `planApprovalMode="require-all"`, or task `requirePlanApproval=true` stops the task at `status: "awaiting-approval"`. A task-level `false` cannot weaken project `require-all`; absent task overrides preserve prior workflow/project behavior.
