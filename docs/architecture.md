...
Native task worktrees are unconditionally task-ID-pinned (`<worktreesDir>/<lowercased-task-id>`); Worktrunk remains responsible for its own layout. Fusion no longer selects random or title-derived directory names and no longer retains completed worktrees in a recycle pool. Workspace tasks use the same rule at their task directory, with one child worktree per repository declared by `.fusion/workspace.json`.
