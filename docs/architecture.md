...
- Active overlap scopes exclude permanently failed `in-review` tasks (`status === "failed"`) so superseding re-implementation tasks are not indefinitely queued behind work that will never merge. Stuck-session recovery itself never creates this terminal shape. (FN-4200/FN-217)
...
**Execution-entry invariant: unplanned/intake-resident cards never enter a processing column (FN-7648).** The hold/release sweep (`runHoldReleaseSweep` in `packages/engine/src/execution/hold-release.ts`, driven from `Scheduler.runHoldReleaseSweepPass`) releases a `hold`-trait card into the nearest downstream `countsTowardWip` column. A card must never be released while it is still unplanned — `status === "planning"`, a pending pre-release Plan Review, a durable replan signal, a duplicate redirect, or a bootstrap-seed PROMPT.md. The automatic sweep first classifies an unplanned or approval-held card as a non-candidate and reports `awaiting-planning:<reason>` or `awaiting-approval`; it does not attempt a release and therefore writes no durable refusal. `issueRelease` remains the defence-in-depth choke point for the sweep, `promoteHeldTask`, and `releaseHeldTaskByEvent`: an explicit non-forced request still records one durable unplanned refusal per episode, and the scheduler's `reserveSlot` callback independently applies the same guard before reserving a worktree/semaphore slot. `allowUnplanned` remains the operator-only plan-gate waiver and is derived from the card's own unplanned state, never from `force` alone; consequently a forced promote of a fully planned approval-held card is still refused, while a genuinely unplanned forced card retains its waiver bookkeeping and still arbitrates capacity. The shared predicate is trait-based rather than keyed to a literal column id, so renamed intake and hold columns receive the same protection. Held cards keep `status` and `blockedBy` untouched and never leak a reservation.
