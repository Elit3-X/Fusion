...
| `worktreeInitCommand` | `string` | `undefined` | Shell command run after task worktree creation and in temporary merge worktrees before merge/review verification. When unset, Fusion infers a package-manager install from the lockfile for fresh and pooled task worktrees (including every workspace sub-repository) and for standalone AI-merge clean rooms; it may skip only when the install marker matches. A configured command remains authoritative and is the opt-out from inference (set a no-op when no install is wanted). Acquisition-time install failure is non-fatal and only degrades the first test run. Useful for project-specific setup beyond package install (for example `pnpm install --frozen-lockfile`, `cp .env.local .env`, or codegen/bootstrap scripts). |
