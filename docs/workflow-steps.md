...
- **Current pre-merge approval:** an enabled Code Review gate permits merge only with a `passed` result carrying an explicit `APPROVE` or `APPROVE_WITH_NOTES` verdict, or a `skipped` result with the audited FN-7720 bypass metadata. Plan-domain gates retain their intentional passed-result behavior. Remediation-archived results are not approvals. A disabled optional group is not a required gate. Diff-domain reviews must match the content being merged: singular tasks use the current diff fingerprint, while workspace tasks require confirmed per-repository `repositoryScope.reviewEvidence` for each modified in-scope repository. A matching plan-review fingerprint proves the plan text, not source diff content, and is never cross-compared with a diff fingerprint. A current workspace `reviewRemediation` record is a durable blocking REVISE. Missing approval clears by rerunning the gate, a failed review can be released through the audited bypass, stale content clears through a fresh review, and unavailable content proof blocks until the gate records new evidence. The corresponding merge blockers are `task has enabled pre-merge workflow steps that never ran`, `task has enabled pre-merge workflow steps without a current approval`, `task has a pre-merge approval recorded against different content`, and `task has no provable approval for the content being merged`.
