...
- The **Activity** view picker offers **Live**, **Feed**, **Raw Logs**, conditional **Interventions**, and **Summaries**. Live, Feed, and Raw Logs share an expand/collapse control that lets the active segment fill the task-detail modal, then restores the normal header, tabs, and action footer when collapsed.
...
- **Activity → Summaries** presents Plan, Code, Review, and Merge as a flat sequential report list with static stage headings and counts—there are no collapsed accordions. Every chronological, dated, markdown-formatted report is visible immediately; Plan, Review, and Merge reuse the same live workflow results as Workflow and load when Summaries opens directly, while Code contains AI-authored summaries recorded when implementation steps finish. A step-session/foreach completion or agent call that omits its summary leaves Code's explicit empty state rather than fabricating a report. Legacy links to History open this Summaries view.
