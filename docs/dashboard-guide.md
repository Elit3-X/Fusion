...
<!-- FNXC:TaskResetDocs 2026-08-27-22:20: Task Reset is a destructive fresh-planning boundary. The guide must explain per-repository cancellation and cleanup, retained history, atomic intake publication, and retry behavior for incomplete cleanup. -->

**Reset** is destructive and has no undo. After confirmation, Fusion fences executor and planner work without waiting for a planner that needs the reset-held planning lock. It removes the task-owned standard worktree, or every task-owned worktree for a workspace task, plus the current `.fusion/tasks/<task-id>/PROMPT.md` plan. For workspace tasks, Reset also removes the now-empty workspace task directory and clears workspace coordination leases and land intents before atomically returning the same task to its workflow's **Planning/intake** column with pending steps and `needs-replan`.

A stale self-owned session registration is reconciled only under the ordinary liveness and idle gates, then removal is retried once. A live planner, executor claim, foreign holder, unsafe path, or unprovable repository ownership is reported as an actionable conflict for the affected repository or the workspace task directory itself; Reset never forces deletion over live work or publishes partial success. If a holder appears at the workspace task directory during repository cleanup, Fusion retains that directory and the current plan, reports incomplete cleanup, and does not publish fresh Planning state. Per-repository task branches are intentionally retained, matching the single-repository Reset behavior.
...
A task created with **Wait for my approval before execution** stops after planning in the workflow's intake or hold planning lane. The preference is available beside **Fast** in Quick Entry and New Task, is scoped to the current project, and stays enabled across submissions until clicked again. A shield badge marks tasks that will request human plan review. When the hold becomes active, **Need Your Review** appears on the Board card, in compact and table List layouts, and as a Task Detail banner. Use **Approve** to continue or **Respecify** to request specific changes; Task Detail keeps **Reject Plan** available for the destructive regenerate-from-scratch path. Respecify and Reject Plan keep the card in place when its workflow plans in the current column rather than relocating it to intake.
...
- The **History** tab presents Plan, Code, Review, and Merge as collapsed accordions with report counts. Expanded stages show chronological, dated, markdown-formatted reports; Plan, Review, and Merge reuse the same live workflow results as Workflow and load when History opens directly, while Code contains AI-authored summaries recorded when implementation steps finish. A step-session/foreach completion or agent call that omits its summary leaves Code's explicit empty state rather than fabricating a report.
