...
- The **Activity** view picker offers **Live**, **Feed**, **Raw Logs**, and conditional **Interventions**. Live, Feed, and Raw Logs share an expand/collapse control that lets the active segment fill the task-detail modal, then restores the normal header, tabs, and action footer when collapsed.
- The **Summary** tab is available for every task and is the default landing tab for completed work. It starts with **Work done by agents**: Plan, Code, Review, and Merge report stages appear in chronological order with static headings and counts—there are no collapsed accordions. Every dated, markdown-formatted report is visible immediately; review reports preserve the reviewer's rationale, multi-repository reviews retain each repository's note, and legacy verdicts with no rationale show an explicit no-notes line. Code includes AI-authored completion summaries when no workflow completion-summary result exists; a missing report stays explicitly empty rather than being fabricated. Summary owns neither spend nor landed-commit facts.
