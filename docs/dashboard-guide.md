...
- **Activity → Summaries** presents Plan, Code, Review, and Merge as a flat sequential report list with static stage headings and counts—there are no collapsed accordions. Every chronological, dated, markdown-formatted report is visible immediately; review reports show the reviewer's rationale, and multi-repository reviews retain each repository's note in its own section. A legacy review with no captured rationale shows an explicit no-notes line instead of the generic missing-body message. Plan, Review, and Merge reuse the same live workflow results as Workflow and load when Summaries opens directly, while Code contains AI-authored summaries recorded when implementation steps finish. A step-session/foreach completion or agent call that omits its summary leaves Code's explicit empty state rather than fabricating a report. Legacy links to History open this Summaries view.
